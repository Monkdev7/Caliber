  ...
  const triggerScrape = async source => {
    try {
      ...
      if (response.data.success) {
        ...
        const jobCount =
          response.data.count || response.data.data?.length || 'New';
        setSuccess(
          `Successfully scraped ${jobCount} jobs from ${source.charAt(0).toUpperCase() + source.slice(1)}!`,
        );
      } else {
      ...
      setError(
        `Failed to scrape ${source} jobs. ${err.response?.data?.message || ''}`,
      );
      console.error(err);
    } finally {
  ...
  const applyFiltersAndSort = () => {
    ...
    if (filters.source && filters.source !== 'all') {
      filtered = filtered.filter(
        job => job.source?.toLowerCase() === filters.source.toLowerCase(),
      );
    }
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        default:
          ...
          compareA = new Date(
            a.updatedAt || a.createdAt || a.scraped_at || a.posted_date || 0,
          );
          compareB = new Date(
            b.updatedAt || b.createdAt || b.scraped_at || b.posted_date || 0,
          );
          break;
      }
  ...
  const exportToCSV = () => {
    ...
    const csvContent = [
      ...
      ...filteredJobs.map(job => {
        ...
        const jobUrl =
          job.jobUrl || job.url || job.job_url || job.link || 'N/A';
    ...
    <div className="min-h-screen bg-slate-950">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
          ...
          {hasActiveFilters() && (
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              {filters.source && filters.source !== 'all' && (
                <span className="inline-flex items-center gap-2 px-3 py-1 bg-slate-900 border border-slate-800 text-slate-300 rounded-full text-sm">
                  Source:{' '}
                  {filters.source.charAt(0).toUpperCase() +
                    filters.source.slice(1)}
                </span>
              )}
            </div>
          )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-lg shadow-md p-4 text-center">
            <div className="text-3xl font-bold text-accent">{jobs.length}</div>
